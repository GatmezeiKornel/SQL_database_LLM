...
def query_gen():
    ...
    if QUERY := st.chat_input("Ide írja a kérdését"):
        ...
        with st.chat_message("assistant"):
            ...
            current_token_count = len(encoding.encode(' '.join([i['content'] for i in sql_gen_messages])))

            while (len(sql_gen_messages)-3 > MAX_CONTEXT_QUESTIONS * 2) or (current_token_count >= MODEL_INPUT_TOKEN_SUMM_LIMIT):

                sql_gen_messages.pop(3)            
                current_token_count = len(encoding.encode(' '.join([i['content'] for i in sql_gen_messages])))

            message_placeholder.markdown(sql_response)

        # Add user and AI message to chat history
        st.session_state.sql_messages.append({"role": "user", "content": QUERY})
        st.session_state.sql_messages.append({"role": "assistant", "content": sql_response})
...
def app_main():
    ...
    page_names_to_funcs = {
    "Talk to your database":talk_to_db,
